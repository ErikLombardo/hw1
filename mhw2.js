...
function NewTrainer(event) {
    ...
    if (form.nomeallenatore.value.length == 0 ||
        ...
        fetch("http://localhost/hw1/AJAX_CaricaTrainer.php", { method: 'post', body: form_data }).then(OnResponse).then(VediErrore);
...
function Aggiungi(event) {
    ...
    fetch("http://localhost/hw1/AJAX_add.php").then(OnResponse).then(Risposta);
...
function Visualizza_allenatori(event) {
    console.log("ssabdul");
    fetch("http://localhost/hw1/AJAX_GetAllenatori.php").then(OnResponse).then(Visualizza_allenatore);
...
function GestisciRicerca(event) {
    ...
    if (argomento.length != 0) {
        fetch("http://localhost/hw1/AJAX_Ricerca.php?Ricerca=" + argomento).then(OnResponse).then(Visualizza_allenatore);
    } else {
        Visualizza_allenatori();
    }
