...
</br> Accedi ora!!! Primo accesso? <a href="registrati.php">Registrati</a></br></br> </span>
<div id="errore">
</div>
...
function risultato(event) {
	...
	fetch("http://localhost/hw1/Accesso.php", {
		method: 'POST',
		body: form_data
...
function Errore(json) {
	...
	if (json == true) {
		...
		window.location.href = 'index.php'; //cambia la location su javascript
	} else {
...
function logout(event) {
	console.log("sono logout");
	fetch("Logout.php").then(LoggedOut);
}
...
function DisplayAccesso(event) {
	...
	fetch("http://localhost/hw1/AJAX_IsLogged.php").then(OnResponse).then(Display);
}
